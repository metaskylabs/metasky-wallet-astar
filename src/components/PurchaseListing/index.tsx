...
const PurchaseListing = (props: {
  ...
  const [state, setState] = useState<{
    state: PurchaseState;
    onBack?: () => void;
  }>({ state: PurchaseState.PREVIEW });
  const { trackClick, trackPage } = useAnalytics();
  const { openWidget } = useOnMetaWidget();

  useEffect(() => {
    trackPage(EVENT_PAGE.PREVIEW_PURCHASE, {
      nft_uuid: props.preview.details.nft.id,
      listing_price: props.preview.details.value,
      currency: props.preview.details.currency,
      order_uuid: props.preview.details?.order_uuid,
      nft_name: props.preview.details.nft.name,
    });
  }, []);

  useEffect(() => {
    setPurchaseData(props.preview);
  }, [props.preview]);
    ...
    try {
      if (purchaseData?.details?.order_uuid) {
        ...
        if (response.data.status === OrderStatus.COMPLETED) {
          ...
          trackEvent(click.purchaseSuccess, {
            listing_uuid: purchaseData.details.reference,
          });
          ...
          state.state !== PurchaseState.ORDER_INPROGRESS
        ) {
          setState((prev) => ({
            ...prev,
            state: PurchaseState.ORDER_INPROGRESS,
          }));
        }
        // else if (
  ...
  const rechargeWallet = async (_orderId: string) => {
    try {
      ...
      /* check via typeguard to select payment mode */
      ...
      setState({
        state: PurchaseState.PROCEED_TO_PAYMENT,
        // onBack: () => {
        //   setState({ ...state });
        // },
      });
      ...
      handleErrorMessage(error);
      if (state.state === PurchaseState.PROCEED_TO_PAYMENT)
        setState({ state: PurchaseState.PREVIEW });
    }
  };
  ...
  const handlePurchase = async () => {
    trackClick(CLICK.PURCHASE_PREVIEW_PURCHASE, {
      listing_uuid: purchaseData?.details.reference,
      order_uuid: purchaseData?.details?.order_uuid,
    });
        ...
        buttonComponent={
          ...
          >
            <SecondaryButton
              ...
              onClick={() => {
                props.onClose();
                trackClick(CLICK.CANCEL_PREVIEW_PURCHASE, {
                  listing_uuid: purchaseData?.details.reference,
                });
                // onBack();
              }}
            ...
            <Fragment>
              ...
              >
                ...
                <TransactionInformation
                  ...
                  nativeFees={
                    purchaseData?.totalFeesNativeIncludingAddons?.value
                  }
                  ...
                  totalAmount={
                    Number(
                      purchaseData?.totalPaymentFiatIncludingAddons,
                    ).toFixed(2) || ``
                  }
                  totalNativeAmount={
                  ...
                  disableOnMetaBanner={purchaseData?.disableOnMetaBanner}
                />
              </motion.div>
            </Fragment>
          </div>
  ...
  const getComponent = () => {
    if (state.state === PurchaseState.KYC) {
      return <KYC setOpenKyc={(status: boolean) => setOpenKyc(status)}></KYC>;
    }
    /* if (state.state === PurchaseState.ORDER_INPROGRESS) {
      return (
        <Success
          title={translate(`YOUR_ORDER_IS_IN_PROGRESS`)}
          ctaText={translate(`TRACK_YOUR_ORDER`)}
          ctaClick={() => router.push(Pages.TRANSACTION)}
          nft_uuid={purchaseData?.details.nft.id}
          listing_price={purchaseData?.details.value}
          currency={purchaseData?.details.currency}
          order_uuid={purchaseData?.details?.order_uuid}
          nft_name={purchaseData?.details.nft.name}
        />
      );
    } */
    if (
      state.state === PurchaseState.ORDER_INPROGRESS ||
      state.state === PurchaseState.PROCEED_TO_PAYMENT
    ) {
      return (
        <Success
          ...
          subTitle={
            state.state === PurchaseState.ORDER_INPROGRESS
              ? `Your transaction is in progress. In case of any issues, click the button below to open payment gateway again.`
              : ``
          }
  ...
  const getTitle = () => {
    if (state.state === PurchaseState.PREVIEW) return translate(`PURCHASE`);
    return ` `;
  };
  ...
  const logCloseEvent = () => {
    ...
    if (title === ` `) {
      trackClick(`Close in Proceed To Payment`, {
        listing_uuid: purchaseData?.details.reference,
      });
    } else {
